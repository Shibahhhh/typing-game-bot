    ...
    async execute(client, message, args) {
        ...
        if(!opponent){
            const embed = new MessageEmbed()
				.setDescription(`${fail} Provide an opponent`)
				.setColor(normal);
        ...
        async function tictactoe(m) {
            ...
            m.edit(`**TicTacToe** | <@!${Args.userid}>'s turn`, buttons)
            ...
            collector.on('collect', b => {
                ...
                if (Object.keys(Brgs).length == 0) return m.edit('Tie!')
                tictactoe(m)
            });
            collector.on('end', collected => {
                if (collected.size == 0) m.edit(`<@!${Args.userid}> didn\'t react in time!`)
            });
        }
